...
<html>
		...
		{   
			switch(frm.measureStatus.value){
				case "0": //可否修改为未估算状态
					...
					break;
				case "1": //可否修改为部分估算状态
					if (editEstimate == -1 || editEstimate == 1)  return true;
					else {
					...
					break;
				case "2": //可否修改为已估算状态
					if (editEstimate == 2||editEstimate == -1)  return true;
					else{
						 alert("此估算状态不能修改为已估算！");
						 frm.measureStatus.focus();
						 return false;
  ...
  <body>
    <%
      ...
      IfpugFunction ifi = (IfpugFunction)rb.getFunctionById(functionId); //获取Function对象
      IfpugFunction function = ifi.getIfpugFunction();//获取其父Function
      Integer fatherId = function.getId();
       //判断估算状态能否改为未估算
  ...
  <form action="/ifpug/IFPUG/editChildFunction.do" name="functionForm" method="post" onSubmit="return verifyInput(<%=editEstimate%>,<%=editType%>);">
     <table>
  		<tr>
  			<td width="15%">模块名称</td>
  			<td >
    ...
    <input type='button' onclick="Javascript:window.location='childFunctionList.jsp?functionId=<%=fatherId %>'" value=" 返 回 ">
  	<input type="hidden" name="id" value="<%=functionId%>"/>
  	<input type="hidden" name="functionId" value="<%=fatherId%>"/>
	</form>
  </body>
</html>
